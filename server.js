...
function verifyPin(pin) {
  if (!/^\d{6}$/.test(String(pin || '')) || !creatorHash) return false;
  const [salt, expected] = creatorHash.split(':');
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/activate') return json(res, 201, store.activate(await read(req)));
    if (req.method === 'GET' && url.pathname.startsWith('/api/sessions/')) return json(res, 200, store.getSession(url.pathname.split('/').pop()));
    if (req.method === 'POST' && url.pathname === '/api/private/unlock') {
      const { pin } = await read(req); if (!verifyPin(pin)) return json(res, 401, { error: 'private access denied' });
      const token = crypto.randomBytes(32).toString('hex'); privateTokens.add(token); return json(res, 200, { token });
    }
